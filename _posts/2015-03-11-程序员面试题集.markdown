...
6、**static**全局变量只初始化一次，防止在其他文件中被引用，局部变量初始化一次，下一次依据上一次的值，static函数内存中只一份。类的static属于类不属于类的实例，可在所有类的实例间共享。类名::静态方法()。  
...
**const**：代替预处理阶段的#define，处于编译器，有数据类型，可进行安全性检查

**inline**：编译、类型检查、函数、二义性；但以代码膨胀为代价，代码较长或循环时慎用
